...
A metodologia empregada neste estudo envolveu uma abordagem de coleta e análise de dados cuidadosamente planejada. Os dados foram coletados por meio de observação direta dos episódios da série "The Office", com foco nas expressões faciais e nas reações emocionais transpassadas nas olhadas para a câmera do personagem Jim Halpert, reconhecidas como formas de comunicação não verbal.

Durante a coleta de dados, foram adotados critérios específicos para a consideração das informações presentes nos olhares de Jim para a câmera. Os eventos foram definidos como aqueles que mobilizaram boa parte do escritório e tiveram impacto perceptível nas dinâmicas de interação dos personagens, como Dia da Diversidade, partidas de futebol e basquete entre departamentos, Halloween, Natal, entre outros. Aniversários individuais foram excluídos desta análise, a menos que fossem notáveis o suficiente para influenciar significativamente a interação geral no escritório por um bom tempo de tela do episódio.

Adicionalmente, para as olhadas que ocorreram em intervalos menores que 15 segundos entre si, foram desconsideradas desde que transmitissem o mesmo sentimento ou contexto emocional. Essa abordagem visa evitar a duplicação de eventos similares e focar nas olhadas que apresentaram variações significativas ou mudanças perceptíveis na expressão facial de Jim.

Para garantir a precisão e consistência dos dados, algumas observações foram validadas por meio de revisão por outras pessoas, assegurando que os sentimentos transmitidos pelas expressões de Jim fossem corretamente interpretados. Cada episódio foi meticulosamente assistido, e as informações relevantes foram catalogadas em um conjunto de colunas que incluíam detalhes como o episódio, temporada, horário, influenciador e se houve a presença de algum evento específico ou brincadeira por parte de Jim envolvida.

É importante observar que, devido à natureza dinâmica da série, algumas olhadas para a câmera podem ter passado despercebidas, e portanto, a análise se baseia na média das olhadas identificadas durante a observação direta.

Posteriormente, os dados coletados foram preparados para análise. Isso envolveu a leitura dos dados utilizando a biblioteca Pandas, seguida pela limpeza e organização dos dados, com exclusão de informações irrelevantes ou inconsistentes. A análise qualitativa dos dados foi realizada utilizando técnicas apropriadas para identificar padrões nas expressões de Jim ao longo da série, com ênfase na apresentação visual dos resultados utilizando a biblioteca Matplotlib em Python.
